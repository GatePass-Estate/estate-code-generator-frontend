...
const TOS_SECTIONS = [
	...
	},
	{
		...
		bullets: ['Be at least 18 years old (or the age of legal majority in your jurisdiction)', 'Have the legal capacity to enter into a binding agreement', 'Use the Service in compliance with all applicable laws and regulations'],
		footer: 'If you are using the Service on behalf of an organisation, you represent that you have the authority to bind that organisation to these Terms.',
	},
	{
		id: 'user-accounts',
		title: '3. User Accounts',
		content: 'To access certain features, you may be required to create an account. You agree to:',
		bullets: ['Provide accurate and complete information', 'Keep your login credentials secure', 'Notify us immediately of any unauthorised access', 'Accept responsibility for all activities under your account'],
	...
	},
	{
		...
		bullets: ['Copy, modify, distribute, or create derivative works', 'Reverse engineer or decompile the software', 'Remove copyright or proprietary notices'],
	},
	{
		id: 'user-content',
		title: '6. User-Generated Content',
		content: 'If you submit, upload, or share content ("User Content"), you grant us a worldwide, royalty-free, non-exclusive license to use, store, reproduce, and display that content solely for operating and improving the Service.\nYou represent that:',
		bullets: ['You own or have rights to the User Content', 'Your content does not violate any laws or third-party rights'],
		footer: 'We reserve the right to remove content that violates these Terms.',
	},
	{
		id: 'privacy',
		title: '7. Privacy and Data Protection',
		content: 'Your use of the Service is also governed by our Privacy Policy and Data Protection Policy, which explain how we collect, use, and protect your personal data.\nWe comply with the Nigeria Data Protection Act (NDPA) 2023 and other applicable laws.',
		customRender: (router: any) => (
			<Text className="font-Inter text-sm text-[#4B5563] leading-6 mb-3">
				...
				<Text onPress={() => router.push('/auth/data-protection-policy')} className="text-primary font-ubuntu-medium underline">
	...
	},
	{
		...
		bullets: ['The content, policies, or practices of third-party services', 'Any loss or damage arising from their use'],
	...
	},
	{
		...
		bullets: ['Uninterrupted access', 'Error-free operation', 'Complete security'],
		footer: 'We may modify, suspend, or discontinue any part of the Service at any time, with or without notice.',
	},
	{
		id: 'termination',
		title: '11. Termination',
		content: 'We may suspend or terminate your access if you:',
		bullets: ['Violate these Terms', 'Engage in harmful or illegal activities', 'Misuse the Service'],
		footer: 'Upon termination:\nYour right to use the Service ends immediately.\nCertain provisions (e.g., intellectual property, disclaimers) will continue to apply.',
	},
	{
		id: 'disclaimers',
		title: '12. Disclaimers',
		content: 'The Service is provided "as is" and "as available" without warranties of any kind, including:',
		bullets: ['Remotely related features not expressly stated', 'Fitness for a particular purpose', 'Non-infringement', 'Accuracy or reliability'],
		footer: 'You use the Service at your own risk.',
	},
	{
		id: 'liability',
		title: '13. Limitation of Liability',
		content: 'To the fullest extent permitted by law, Devault is not liable for:',
		bullets: ['Indirect, incidental, or consequential damages', 'Loss of data, profits, or business', 'Unauthorised access to your information', 'Errors or interruptions in the Service'],
		footer: 'Our total liability will not exceed the amount you paid for the Service in the last 12 months (if applicable).',
	},
	{
		id: 'indemnification',
		title: '14. Indemnification',
		content: 'You agree to indemnify and hold Devault harmless from any claims, damages, or losses arising from:',
		bullets: ['Your use of the Service', 'Your violation of these Terms', 'Your infringement of third-party rights'],
	},
	{
		id: 'governing-law',
	...
	},
	{
		...
		bullets: ['We will update the "Last Updated" date', 'Continued use of the Service constitutes acceptance of the revised Terms'],
	},
	{
		id: 'contact',
		title: '17. Contact Information',
		content: 'For questions or concerns about these Terms, contact us at:\ninfo@gatepassng.com',
...
export default function TermsOfService() {
			...
			<SafeAreaView className="h-full bg-white">
				<View className="flex-row flex-1">
					<View className="w-72 border-r border-[#E5E7EB] p-8 justify-between">
						<View>
							...
							<View className="mt-12 gap-2">
								{TOS_SECTIONS.map((section) => (
									<Pressable key={section.id} onPress={() => handleSectionPress(section.id)} className="flex-row items-center">
										<View
						...
						<Pressable onPress={handleBackFromTerms} className="border border-[#D1D5DB] rounded-lg py-3 px-6 items-center mt-8">
					...
					<ScrollView ref={scrollViewRef} className="flex-1" contentContainerStyle={{ padding: 48, paddingBottom: 80 }} onScroll={handleScroll} scrollEventThrottle={16} showsVerticalScrollIndicator={false}>
						<Text className="font-UbuntuSans text-4xl text-primary">Terms of Service</Text>
						<Text className="font-Inter text-base text-orange mt-2">Last Updated: 20th March 2026.</Text>

						<View className="bg-[#F8F8F8] rounded-2xl p-8 mt-8">
							{TOS_SECTIONS.map((section, index) => (
								<View key={section.id} onLayout={(e) => handleSectionLayout(section.id, e.nativeEvent.layout.y)} className={index > 0 ? 'mt-8' : ''}>
									<Text className="font-UbuntuSans text-xl text-black font-semibold">{section.title}</Text>

									{section.customRender ? section.customRender(router) : <Text className="font-Inter text-sm text-[#4B5563] mt-3 leading-5">{section.content}</Text>}
										...
										<View className="mt-2 ml-4">
											{section.bullets.map((bullet, i) => (
												<View key={i} className="flex-row mt-1">
													...
													<Text className="font-Inter text-sm text-[#4B5563] flex-1">{bullet}</Text>
												</View>
											))}
										</View>
									)}
									{section.footer && <Text className="font-Inter text-sm text-[#4B5563] mt-3 leading-5">{section.footer}</Text>}
									{section.email && (
										<Pressable onPress={() => Linking.openURL(`mailto:${section.email}`)} className="mt-2">
											<Text className="font-Inter text-sm text-primary underline">Email us</Text>
										</Pressable>
									)}
								</View>
							))}
						...
						{!isReadOnly && (
							<View className="mt-10 gap-4 max-w-2xl self-center w-full">
								<Button className="rounded-lg h-14 items-center justify-center" size={Platform.select({ ios: 'lg', default: 'lg' })} onPress={handleAccept} disabled={isAccepting}>
									{isAccepting ? <ActivityIndicator color="#fff" /> : <Text className="text-white font-UbuntuSans font-semibold text-base">I Accept</Text>}
								</Button>
								<Button className="rounded-lg h-14 items-center justify-center bg-teal" size={Platform.select({ ios: 'lg', default: 'lg' })} onPress={handleReject} disabled={isRejecting}>
									{isRejecting ? <ActivityIndicator color="#fff" /> : <Text className="text-white font-UbuntuSans font-semibold text-base">I Reject</Text>}
								</Button>
							</View>
						)}
					</ScrollView>
	...
	return (
		<SafeAreaView className="flex-1 bg-white">
			...
			<ScrollView ref={scrollViewRef} contentContainerStyle={{ padding: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
				<Text className="font-ubuntu-medium text-2xl text-black mt-6">Terms of Service</Text>
				<Text className="font-Inter text-sm text-orange mt-1">Last Updated: 20th March 2026.</Text>

				<View className="bg-[#F8F8F8] rounded-2xl p-5 mt-6">
					{TOS_SECTIONS.map((section, index) => (
						<View key={section.id} onLayout={(e) => handleSectionLayout(section.id, e.nativeEvent.layout.y)} className={index > 0 ? 'mt-6' : ''}>
							<Text className="font-UbuntuSans text-lg text-black font-semibold">{section.title}</Text>

							{section.customRender ? section.customRender(router) : <Text className="font-Inter text-sm text-[#4B5563] mt-2 leading-5">{section.content}</Text>}
				...
				{!isReadOnly && (
					<View className="mt-8 gap-4 items-center">
						<TouchableOpacity onPress={handleAccept} disabled={isAccepting} style={{ backgroundColor: '#113E55', borderRadius: 10, height: 56, alignItems: 'center', justifyContent: 'center', width: '83%' }} activeOpacity={0.8}>
							{isAccepting ? <ActivityIndicator color="#fff" /> : <Text className="text-white font-UbuntuSans font-semibold text-base">I Accept</Text>}
						</TouchableOpacity>
						<TouchableOpacity onPress={handleReject} disabled={isRejecting} style={{ backgroundColor: '#1B998B', borderRadius: 10, height: 56, alignItems: 'center', justifyContent: 'center', width: '83%' }} activeOpacity={0.8}>
							{isRejecting ? <ActivityIndicator color="#fff" /> : <Text className="text-white font-UbuntuSans font-semibold text-base">I Reject</Text>}
						</TouchableOpacity>
					</View>
				)}
