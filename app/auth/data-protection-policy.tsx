		...
		return (
			<SafeAreaView className="flex-1 bg-white flex-row">
				<View className="w-[320px] bg-white border-r border-[#E5E7EB] p-8 h-full">
						...
						<View>
							{POLICY_SECTIONS.map((section) => (
								<Pressable key={section.id} onPress={() => handleSectionPress(section.id)} className={`py-2 px-3 rounded-lg flex-row items-center mb-1 transition-colors`}>
									{activeSection === section.id && <View className="w-1.5 h-1.5 rounded-full bg-primary mr-2" />}
									<Text className={`font-UbuntuSans text-sm ${activeSection === section.id ? 'text-primary font-bold' : 'text-[#4B5563]'}`} numberOfLines={1}>
										{section.title}
									</Text>
								</Pressable>
							))}
					...
					<Pressable onPress={() => router.back()} className="border border-[#D1D5DB] rounded-lg py-3 px-6 items-center">
					...
					<View className="bg-[#F8F8F8] rounded-2xl p-8 mt-8">
						{POLICY_SECTIONS.map((section, index) => (
							<View key={section.id} onLayout={(e) => handleSectionLayout(section.id, e.nativeEvent.layout.y)} className={index > 0 ? 'mt-8' : ''}>
								<Text className="font-UbuntuSans text-2xl text-black font-semibold mb-3">{section.title}</Text>
								{section.content ? <Text className="font-Inter text-sm text-[#4B5563] leading-6 mb-3">{section.content}</Text> : null}
									...
									<View className="mb-3">
										{section.bullets.map((bullet, idx) => (
											<View key={idx} className="flex-row mb-2 pr-4">
												...
												<Text className="font-Inter text-sm text-[#4B5563] leading-6 flex-1">{bullet}</Text>
											</View>
										))}
									</View>
								)}
								...
								{section.subSections &&
									section.subSections.map((sub, sIdx) => (
										<View key={sIdx} className="mt-4 mb-2 ml-4">
											<Text className="font-UbuntuSans text-lg text-black font-medium mb-2">{sub.title}</Text>
											{sub.content && <Text className="font-Inter text-sm text-[#4B5563] leading-6 mb-2">{sub.content}</Text>}
											{sub.bullets && (
												<View className="mt-2">
								...
								{section.footer && <Text className="font-Inter text-sm text-[#4B5563] leading-6 mt-2">{section.footer}</Text>}
							</View>
						))}
					</View>

					<View className="flex-row justify-end mt-12 mb-8">
						<Pressable onPress={() => router.back()} className="bg-primary rounded-lg py-3 px-8">
							<Text className="text-white font-UbuntuSans font-semibold text-base">{fromSettings ? 'Back' : 'Return to Terms of Service'}</Text>
						</Pressable>
					</View>
				</ScrollView>
			</SafeAreaView>
		...
		<SafeAreaView className="flex-1 bg-white">
			...
			<ScrollView ref={scrollViewRef} className="flex-1" contentContainerStyle={{ padding: 20, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
				...
				<View className="bg-[#F8F8F8] rounded-xl p-6">
					{POLICY_SECTIONS.map((section, index) => (
						<View key={section.id} onLayout={(e) => handleSectionLayout(section.id, e.nativeEvent.layout.y)} className={index > 0 ? 'mt-5' : ''}>
							<Text className="font-UbuntuSans text-xl text-black font-semibold mb-1">{section.title}</Text>
							{section.content ? <Text className="font-Inter text-sm text-[#4B5563] leading-6">{section.content}</Text> : null}

							{section.bullets && (
								<View className="">
									{section.bullets.map((bullet, idx) => (
										<View key={idx} className="flex-row mb-1 pr-2">
											<Text className="text-[#4B5563] mr-2 text-sm leading-6">•</Text>
											<Text className="font-Inter text-sm text-[#4B5563] leading-6 flex-1">{bullet}</Text>
										</View>
									))}
							...
							{section.subSections &&
								section.subSections.map((sub, sIdx) => (
									<View key={sIdx} className="ml-2">
										<Text className="font-UbuntuSans text-base text-black font-medium mb">{sub.title}</Text>
										{sub.content && <Text className="font-Inter text-sm text-[#4B5563] leading-6 mb-1">{sub.content}</Text>}
										{sub.bullets && (
											<View className="">
												{sub.bullets.map((bullet, idx) => (
				...
				<View className="mt-10 mb-8 w-full">
					<Pressable onPress={() => router.back()} className="bg-primary rounded-lg py-4 w-full items-center justify-center flex-row">
						<Text className="text-white font-UbuntuSans font-semibold text-base">{fromSettings ? 'Back' : 'Return to Terms'}</Text>
					</Pressable>
				</View>
			</ScrollView>
		</SafeAreaView>
