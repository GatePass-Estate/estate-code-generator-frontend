				...
				<View className="bg-[#F8F8F8] rounded-xl p-6">
					{POLICY_SECTIONS.map((section, index) => (
						<View key={section.id} onLayout={(e) => handleSectionLayout(section.id, e.nativeEvent.layout.y)} className={index > 0 ? 'mt-5' : ''}>
							<Text className="font-UbuntuSans text-xl text-black font-semibold mb-3">{section.title}</Text>
							{section.content ? <Text className="font-Inter text-sm text-[#4B5563] leading-6 mb-3">{section.content}</Text> : null}
							...
							{section.subSections &&
								section.subSections.map((sub, sIdx) => (
									<View key={sIdx} className="mt-3 ml-2">
										<Text className="font-UbuntuSans text-base text-black font-medium mb-1">{sub.title}</Text>
										{sub.content && <Text className="font-Inter text-sm text-[#4B5563] leading-6 mb-2">{sub.content}</Text>}
										{sub.bullets && (
											<View className="mt-1">
												{sub.bullets.map((bullet, idx) => (
