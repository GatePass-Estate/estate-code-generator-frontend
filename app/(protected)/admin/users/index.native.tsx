      ...
      <View className="flex-1">
        ...
        <View className="flex-row items-center bg-gray-100 rounded-xl px-4 py-3 mb-4 ">
          ...
          <TextInput
            ...
            className="flex-1 ml-3 text-primary font-inter-regular"
          />
        </View>
        ...
        <View className="flex-row gap-2 mb-6 w-full">
          ...
          >
            <Image
              source={icons.adminHomeIcon}
              style={{
                width: 20,
                height: 20,
                opacity: selectedRole === 'resident' ? 1 : 0.4,
                tintColor: selectedRole === 'resident' ? undefined : '#999',
              }}
            />
          </Pressable>

          <Pressable
            className={`flex-1 justify-center items-center py-3 rounded-2xl border ${selectedRole === 'security' ? 'bg-teal/10 border-teal' : 'bg-white border-gray-300'}`}
            onPress={() => setSelectedRole(selectedRole === 'security' ? null : 'security')}
          >
            <Image
              ...
              style={{
                width: 20,
                height: 20,
                opacity: selectedRole === 'security' ? 1 : 0.4,
                tintColor: selectedRole === 'security' ? undefined : '#999',
              }}
            />
          </Pressable>
          ...
          >
            <Image
              source={icons.activeAdminIcon}
              style={{
                width: 17,
                height: 20,
                opacity: selectedRole === 'admins' ? 1 : 0.4,
                tintColor: selectedRole === 'admins' ? undefined : '#999',
              }}
            />
          </Pressable>
          ...
          >
            <Feather
              name="user-x"
              size={20}
              color={selectedRole === 'unverified' ? '#ef4444' : '#999'}
            />
          </Pressable>
        </View>
        {isLoading ? (
          ...
          <FlatList
            ...
            ListFooterComponent={
              isFetchingMore ? (
                <ActivityIndicator size="small" color="#113E55" style={{ marginVertical: 20 }} />
              ) : null
            }
            renderItem={({ item, index }) => (
              <TouchableOpacity
                className={`flex-row items-center py-4 ${index !== displayedUsers.length - 1 ? 'border-b border-gray-200' : ''}`}
                onPress={() =>
                  router.push({
              ...
              >
                <Image
                  ...
                  style={{
                    width: getRoleIconWidth(item.role),
                    height: getRoleIconHeight(item.role),
                  }}
                />
