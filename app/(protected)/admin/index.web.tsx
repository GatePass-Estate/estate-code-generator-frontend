...
function AdminUsersPageWeb() {
  ...
  const [selectedRole, setSelectedRole] = useState<UserRolesType | 'admins' | 'unverified' | null>(
    null
  );
  ...
  const filteredUsers = useMemo(() => {
    return users.items.filter((user) => {
      ...
      if (selectedRole === 'admins') {
        matchesRole = Boolean(
          user.status && user.role && ['admin', 'primary_admin'].includes(user.role as string)
        );
    ...
    <div className="flex h-full w-screen overflow-y-scroll bg-body">
              ...
              <div className="grid grid-cols-12 gap-4 mb-8">
                ...
                <div className="border border-primary rounded-xl p-6 flex items-center justify-center col-span-1 md:col-span-2">
                  <div className="text-center">
                    <p className="text-grey text-2xl font-ubuntu-regular uppercase tracking-wide">
                      Total
                    </p>
                ...
                <div className="flex items-center justify-center col-span-1 md:col-span-2">
                  <button
                    className="bg-grey/5 rounded-full p-4"
              ...
              <div className="mb-8">
                ...
                <div className="flex gap-4 items-end">
                  <div className="flex flex-1 gap-0">
                    ...
                    <button className="bg-primary text-white px-3 py-3 rounded-lg font-medium hover:opacity-90 transition rounded-l-none">
                      Go
                    </button>
                    ...
                    <tbody>
                      {filteredUsers.map((user, index) => (
                        ...
                        >
                          <td className="p-4">
                            <Image
                              source={getRoleIcon(user.role)}
                              style={{
                                width: getRoleIconWidth(user.role),
                                height: getRoleIconHeight(user.role),
                              }}
                              resizeMode="contain"
                            />
                          </td>
                            ...
                            <div className="flex items-center">
                              {user.status ? (
                                <>
                                  {user.role === 'security' ? (
                                    <button
                                      className="p-2 border-r-2 border-transparent opacity-0 pointer-events-none"
                                      disabled
                                    >
                                    ...
                                    <button
                                      ...
                                      className="p-2 hover:bg-gray-200 transition border-r-2 border-grey"
                                      title="Make Admin"
                                    >
                                      <Image
                                        source={icons.userIcon}
                                        style={{ width: 20, height: 20, tintColor: '#113E55' }}
                                        resizeMode="contain"
                                      />
                                    </button>
                                  ) : (
      ...
      {modalState.isOpen && (
        <Modal
          ...
          actionText={
            modalState.actionType === 'promote'
              ? 'Promote'
              : modalState.actionType === 'demote'
                ? 'Demote'
                : modalState.actionType === 'deactivate'
                  ? 'Deactivate'
                  : modalState.actionType === 'reactivate'
                    ? 'Reactivate'
                    : modalState.actionType === 'resendVerification'
                      ? 'Resend'
                      : modalState.actionType === 'deleteUser'
                        ? 'Delete'
                        : undefined
          }
          actionBtnClassName={
            ['promote', 'reactivate', 'resendVerification'].includes(modalState.actionType || '')
