...
const EditRequestMobile = () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to refresh edit requests";
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const requestDetails = await Promise.all(
        selectedRequests.map((r) => getRequestById(r.id)),
      );
      ...
      await Promise.all(
        requestDetails.map((req) => {
          if (!req.new_value) {
            throw new Error(
              `Cannot approve request ${req.id}: new value is missing`,
            );
          }
      ...
      setToastMessage(
        `${selectedRequests.length} request(s) approved successfully!`,
      );
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      setToastMessage(
        `${selectedRequests.length} request(s) declined successfully!`,
      );
      setToastType("success");
      setToastVisible(true);
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <Text className="text-2xl mt-10 font-ubuntu-bold text-primary mb-8">
          Edit Request
        </Text>
  ...
  return (
    ...
    >
      ...
      <Text className="text-2xl mt-10 font-ubuntu-bold text-primary mb-6">
        Edit Request
      </Text>
      ...
      {requests.length > 0 && (
        <Pressable
          ...
          className="flex-row items-center gap-3 p-3 pl-0 bg-grey-50 rounded-lg"
        >
          <View
            className={`w-6 h-6 rounded border-2 border-grey items-center justify-center ${allSelected ? "bg-grey" : "bg-white"}`}
          >
            {allSelected && <Icon name="checkmark" size={16} color="#fff" />}
          </View>
          <Text className="text-grey font-ubuntu-semibold flex-1">
        ...
        renderItem={({ item }) => (
          <View className="flex-row items-center gap-3 my-5 rounded-lg active:bg-grey-50">
            <Pressable
              ...
              className={`w-6 h-6 rounded border-2 border-grey items-center justify-center ${item.selected ? "bg-grey" : "bg-white"}`}
            >
              {item.selected && (
            ...
            <View className="flex-1">
              <Text className="text-grey font-inter-regular text-md">
                {item.userName}
              </Text>
              <Text className="text-grey text-sm font-inter-regular">
                {item.location}
              </Text>
            </View>
      ...
      {requests.length > 0 && (
        <View className="flex-row gap-3 mt-8">
          <TouchableOpacity
            ...
            className={`flex-1 bg-teal justify-center items-center py-6 rounded-xl ${processing ? "opacity-70" : ""}`}
            activeOpacity={0.8}
          >
          ...
          <TouchableOpacity
            ...
            className={`flex-1 bg-primary justify-center items-center py-6 rounded-xl ${processing ? "opacity-70" : ""}`}
            activeOpacity={0.8}
          >
