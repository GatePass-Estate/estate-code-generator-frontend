  ...
  return (
    <View className="flex-1 bg-page px-6 pt-6">
      ...
      <View className="flex-row items-center border-b border-border pb-4 mb-4">
        ...
        <Text className="w-24 text-text-muted text-xs uppercase text-right">
          Status
        </Text>
      </View>
      ...
      <ScrollView>
        {MOCK_REQUESTS.map((item) => (
          ...
          >
            ...
            {/* Date */}
            <Text className="w-24 text-text-secondary text-sm">
              {item.date}
            </Text>
            ...
            <View className="w-24 items-end">
              <View
                className={`px-3 py-1.5 rounded-full ${
                  item.status === "Approved" ? "bg-highlight" : "bg-muted"
                }`}
              >
