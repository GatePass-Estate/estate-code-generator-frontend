...
function EditRequestsWeb() {
  ...
  useEffect(() => {
    if (Platform.OS === "web")
      document.title = "Edit Requests - Admin Access - GatePass";
  }, []);
  ...
  const toggleSelectAll = () => {
    ...
    setEditRequests((prev) =>
      prev.map((request) => ({ ...request, selected: !allSelected })),
    );
  };

  const selectedRequests = editRequests.filter((r) => r.selected);
  const allSelected =
    editRequests.length > 0 && editRequests.every((r) => r.selected);
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "An error occurred while approving requests";
      setMessageType("error");
      setError(errorMessage);
    } finally {
      setRunning(false);
  ...
  const handleDeclineSelected = async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "An error occurred while declining requests";
      setMessageType("error");
      setError(errorMessage);
    } finally {
      setRunning(false);
  ...
  const handleApproveFromModal = async () => {
    ...
    try {
      ...
      setEditRequests(
        editRequests.filter((req) => req.id !== selectedRequest.id),
      );
      setSelectedRequest(null);
      ...
      <div className="web-body pb-20">
        <div className="mt-20 mb-10">
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-5xl font-ubuntu-regular text-grey">
              Admin Access
            </h1>
          </div>
          ...
          <div className="grid grid-cols-4 gap-9 md:grid-cols-12 mb-10">
            ...
            <div className="col-span-10">
              <div className="mt-6">
                <h2 className="text-2xl font-ubuntu-medium text-primary">
                  Edit Request
                </h2>
