    ...
    <div className="flex h-full w-screen overflow-y-scroll bg-body">
      ...
      <div className="web-body pb-20">
              ...
              {GENDERS.map((g, index) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setSelectedGender(g.value as GenderType)}
                  >
              ...
              {RELATIONSHIPS.map((r, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`flex flex-row items-center px-4 py-2 rounded-md bg-light-grey ${active && 'bg-[#e6f4ef] border border-[##cfe7db]'} gap-3 cursor-pointer`}
                    onClick={() => setSelectedRelationships(r.value as RelationshipType)}
                  >
        ...
        <div className={`mt-7 flex flex-row justify-end gap-3 ${!isLargeScreen && '!mb-32'}`}>
          <button
            className={`bg-dark-teal rounded-md px-10 py-3 ${running && 'cursor-not-allowed opacity-75'}`}
            disabled={running}
          ...
          <button
            className={`bg-primary rounded-md px-10 py-3 ${running && 'cursor-not-allowed opacity-75'}`}
            disabled={running}
