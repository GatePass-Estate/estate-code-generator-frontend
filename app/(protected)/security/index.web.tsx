    ...
    >
      ...
      <div className="relative w-full max-w-[700px] rounded-md shadow-[0_30px_50px_rgba(3,10,14,0.6)] bg-primary">
        <button
          ...
          aria-label="Close"
          className="absolute right-12 top-10 text-body text-[22px] bg-transparent border-none cursor-pointer p-2 leading-none"
        >
          ✕
        </button>
            ...
            <>
              ...
              <div className="bg-accent text-primary rounded-lg p-5 mb-6">
                <div className="flex justify-between items-start py-2 px-1">
                  ...
                  <div className="text-right text-[14px] capitalize">
                    {result?.visitor_fullname}
                  </div>
                </div>
                <div className="flex justify-between items-start py-2 px-1">
                  <div className="w-40 text-[14px]">Gender :</div>
                  <div className="text-right text-[14px] capitalize">
                    {result?.gender == 'prefer_not_to_say' ? 'Prefer Not To Say' : result?.gender}
                  </div>
                </div>
                <div className="flex justify-between items-start py-2 px-1">
                  <div className="w-40 text-[14px]">Relationship :</div>
                  <div className="text-right text-[14px] capitalize">
                    {result?.relationship_with_resident}
                  </div>
                </div>
              </div>
            </>
...
function SearchCode({
  ...
  setResult: (data: GuestDetails) => void;
  setModalOpen: (open: boolean) => void;
}) {
  const inputCount = 6;
    ...
    <main className="text-center mt-4 py-12">
      ...
      <div className="flex justify-center gap-2 mb-7" aria-label="Guest code">
        {values.map((val, i) => (
          <label
            key={i}
            className="w-14 h-14 sm:w-16 sm:h-20 bg-white border border-gray-300 rounded-lg flex items-center justify-center shadow-sm"
          >
            <input
              ref={(el) => {
      ...
      <button
        ...
        className={`bg-primary text-white py-4 px-20 rounded-lg text-sm shadow-lg font-semibold mt-20 transition-all ${isValidating ? 'opacity-70 cursor-not-allowed' : 'hover:opacity-90'}`}
      >
  ...
  return (
    <main className="text-center py-12" role="main" aria-labelledby="profile-title">
      ...
      <div className="mb-8 w-[560px]">
        <section
          className="bg-primary text-white p-6 sm:p-10 rounded-lg max-w-full shadow-lg"
        ...
        <div className="flex flex-wrap justify-between gap-4 mt-8">
          <button
            ...
            aria-label="Log out"
            className="bg-white text-[#123b47] border border-accent rounded-lg py-3 px-7 text-sm hover:bg-gray-100 transition flex-grow"
          >
            Log Out
          </button>
          ...
          <button
            ...
            className="bg-dark-teal text-white rounded-lg py-3 px-7 text-sm hover:opacity-90 transition flex-grow"
          >
            Change Password
          </button>
          ...
          <button
            onClick={handleEdit}
            aria-label="Edit request"
            className="bg-primary text-white rounded-lg py-3 px-7 text-sm hover:opacity-90 transition flex-grow"
          >
            Edit Request
          </button>
        </div>
      ...
      <div className="mx-auto p-4 px-20 flex flex-col min-h-screen">
        <header className="flex items-center justify-center mt-5 pb-10">
          ...
          <nav className="flex gap-4 items-center">
            <span
              onClick={() => setActiveTab('validateCode')}
              className={`text-primary text-sm px-4 py-2 font-semibold border-primary transition-all ${activeTab === 'validateCode' ? 'border-b-2 border-primary cursor-default font-bold' : 'cursor-pointer'}`}
            >
              Validate Code
            </span>
            ...
            <span
              ...
              className={`text-primary text-sm px-4 py-2 font-semibold border-primary transition-all ${activeTab === 'profile' ? 'border-b-2 border-primary cursor-default font-bold' : 'cursor-pointer'}`}
            >
              My Profile
            </span>
          </nav>
        ...
        <div className="mt-10">
          {activeTab === 'validateCode' ? (
            <SearchCode setResult={setResult} setModalOpen={setModalOpen} />
          ) : (
            <Profile />
          )}
        </div>
