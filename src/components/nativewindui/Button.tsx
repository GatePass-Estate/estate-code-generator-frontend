...
import { TextClassContext } from '@/src/components/nativewindui/Text';
...
const Button = React.forwardRef<React.ElementRef<typeof Pressable>, ButtonProps>(({ className, variant = 'primary', size, style = BORDER_CURVE, androidRootClassName, ...props }, ref) => {
	const { colorScheme } = useColorScheme();

	// Only compute android ripple when running on Android and when the expected keys exist.
	const androidRipple = Platform.OS === 'android' ? ANDROID_RIPPLE[colorScheme]?.[variant] : undefined;
	...
	return (
		<TextClassContext.Provider value={buttonTextVariants({ variant, size })}>
			...
			>
				<Pressable className={cn(props.disabled && 'opacity-50', buttonVariants({ variant, size, className }))} ref={ref} style={style} {...(androidRipple ? { android_ripple: androidRipple } : {})} {...props} />
			</Root>
