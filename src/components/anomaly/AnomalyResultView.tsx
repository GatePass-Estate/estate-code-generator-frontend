...
export default function AnomalyResultView({ isActive = true }: { isActive?: boolean }) {
  ...
  const [gaugeSectionLayout, setGaugeSectionLayout] = useState<{
    y: number;
    height: number;
  } | null>(null);
  ...
  const gaugeList: GaugeData[] = React.useMemo(() => {
    const factors =
      overview?.anomaly_overview?.contributing_factors ||
      overview?.anomaly_overview?.top_contributing_factors;
    ...
    return factors.map((factor: any, index: number) => {
      ...
      const formattedWeight =
        weight != null ? `${Number(weight).toFixed(weight === 0 ? 0 : 2)}%` : '-';
        ...
        <View style={{ marginBottom: 32, marginTop: 24 }}>
          ...
          >
            ...
            {predictions.length === 0 ? (
              ...
              (sortAscending ? predictions : [...predictions].reverse()).map(
                (row: any, index: number) => {
                  ...
                  return (
                    <Pressable
                      ...
                      onPress={() =>
                        router.push(
                          `/(protected)/(shared-screens)/ai-store/anomaly-detection/user/${row.prediction_id || row.id}?gender=${row.gender || ''}&user_type=${row.user_type || ''}&display_name=${encodeURIComponent(row.display_name || row.name || '')}`
                        )
                      }
                      style={{
                        flexDirection: 'row',
            ...
            {(() => {
              ...
              if (spider && spider.length > 0) {
                return (
                  <AnomalyRadarChart
                    size={300}
                    labels={overview.anomaly_overview.spider_plot.map(
                      (p: any) => p.feature_name || ''
                    )}
                    series={[
                      {
                        data: overview.anomaly_overview.spider_plot.map(
                          (p: any) => p.percentage || 0
                        ),
                        strokeColor: '#1B998B',
                ...
                return topFactors.map((factor: any, index: number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text
                        allowFontScaling={false}
                        style={{ fontFamily: 'Inter_18pt-Medium', fontSize: 11.5, color: color }}
                      >
                        {factor.feature_name
                          ?.split('_')
                          .map((w: string) => w.charAt(0).toUpperCase() + w.slice(1))
          ...
          >
            ...
            >
              ...
              <View style={{ flex: 1, justifyContent: 'flex-end', gap: 4 }}>
                ...
                >
                  Anomalous entry time was detected in{' '}
                  {overview?.evidence_summary?.total_anomalous_residents_instances || 0} resident
                  records.
                </Text>
              </View>
            </View>
            ...
            >
              ...
              <View style={{ flex: 1, justifyContent: 'flex-end', gap: 4 }}>
                ...
                >
                  Anomalous entry time was detected in{' '}
                  {overview?.evidence_summary?.total_anomalous_visitors_instances || 0} visitor
                  records.
                </Text>
              </View>
            </View>
