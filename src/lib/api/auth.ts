...
export const MY_PROFILE_QUERY_KEY = queryKeys.myProfile;
...
export async function acceptTos(token: string): Promise<LoginResponse> {
  try {
    ...
    throw new Error(
      `${getErrorMessage(error) || "Failed to accept Terms of Service"} `,
    );
  }
}
...
async function verifyEmailActivationToken(
  token: string,
): Promise<VerifyEmailActivationResponse> {
  try {
    const api = Api("user");
    const response = await api.get<VerifyEmailActivationResponse>(
      `/users/verify/email`,
      {
        params: { token },
      },
...
export async function verifyPasswordReset(
  ...
  try {
    ...
    const axiosRes = await api.get(
      `/users/verify/password-reset?token=${token}`,
    );
    return axiosRes.data;
  } catch (error: any) {
    throw new Error(
...
export async function resetPassword(
  ...
  try {
    const api = Api();
    const axiosRes = await api.post(`/users/password/reset`, {
      user_id,
      new_password,
    });
    return axiosRes.data;
